			...
			dbScenario(t, func(tx *gorm.DB) {
				...
				moretest.Suite(
					...
					func(t *testing.T) {
						...
						handler := warehouse_service.NewWarehousePushHandler(tx, nil)
						...
						var histories []warehouse_models.DailySkuHistory
						err = tx.Where("sku_id = ?", "11111111").Find(&histories).Error
						assert.NoError(t, err)
